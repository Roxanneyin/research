...
date <- seq(as.Date("2014-09-15"), by = "day", length.out = 300)#adjust the date 1
...
for (n in 1:length(g_num)){
  NHL <- matrix(0,40,116)
  NHL_Template <- read_excel("D:/ice_hockey_data/NHL Template.xlsx", sheet = "Sheet1", range = "A2:DL2", col_names = FALSE)
  ...
  NHL[,1] = paste('http://www.nhl.com/scores/htmlreports/20142015/GS', substr(g_num[n],5,10), ".HTM",sep = '')#adjust the date 2
  ...
  if (length(sc_n)+length(sk_n)+length(g_n)!=0){
    web3<-paste("http://www.nhl.com/scores/htmlreports/20142015/GS",substr(g_num[n],5,10),".HTM",sep = "")#adjust the date 3
    web3<-read_html(web3)
  ...
  write.csv(NHL,file=paste("d:/ice_hockey_data/2014-2015/",g_num[n],".csv",sep = ""))#adjust the date 4
}
